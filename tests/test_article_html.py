...
case "${1:-}" in
  */prepare_public_knowledge.py) printf '%s\\n' "$V8STD_REPO_ROOT/public.toml" ;;
            ...
            for inherited_path in (None, '/existing/modules'):
                with self.subTest(check_exit=check_exit, pythonpath=inherited_path):
                    with tempfile.TemporaryDirectory() as directory:
                        ...
                        build_index = calls.index(f'-m zensical build --config-file {content}/public.toml --strict')
                        ...
                        publishers = [
                            ...
                            f'{scripts}/generate_ai_artifacts.py --write-public-ai {content}/site',
                            f'{scripts}/publish_public_knowledge.py --site {content}/site',
                        ]
                        cleanup = f'{scripts}/prepare_public_knowledge.py --root {content} --clean'
                        self.assertEqual(calls[build_index + 2:], (publishers if check_exit == 0 else []) + [cleanup])
