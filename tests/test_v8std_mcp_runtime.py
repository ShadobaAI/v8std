...
class RuntimeTests(unittest.TestCase):
    def test_fork_retrieval_uses_one_generation_and_collection_filters(self):
        runtime = self.module()
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(facade.search("std437", collections=["corporate"])["results"], [])
            for method, args in (
                ("summary", (LOCAL + "std/437/",)),
                ("section", ("std437", "missing heading")),
                ("pattern", ("std437",)),
                ("api_card", ("std437",)),
                ("related_ids", ("std437",)),
                ("requirements_for_context", ("std437",)),
            ):
                with self.subTest(method=method):
                    before = current.calls
                    result = getattr(facade, method)(*args)
                    self.assertEqual(current.calls, before + 1)
                    if method == "summary":
                        self.assertTrue(result["found"])
                        self.assertEqual(result["page"]["url"], LOCAL + "std/437/")
                    elif method == "requirements_for_context":
                        self.assertEqual(result["requirements"][0]["id"], "std437")
                    elif method in {"pattern", "api_card", "section"}:
            ...
            with tempfile.TemporaryDirectory() as directory:
                ...
                with TestClient(server.streamable_http_app()) as client:
                    ...
                    for number in range(2):
                        ...
                        self.assertEqual(len(response.json()["result"]["tools"]), 11)
