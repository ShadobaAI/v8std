...
from runtime.v8std_mcp_presentation import LinkCatalog, present_result
from runtime.v8std_mcp_snapshot_format import DEFAULT_SITE_URL, VerifiedSnapshot, normalize_site_url
from runtime.v8std_mcp_snapshots import LoaderError, SnapshotCoordinator, SnapshotStore
from runtime.v8std_mcp_hold import CONTROL_PATH
...
class SnapshotIndex:
    ...
    def search(self, query, *, types=None, collections=None, mode="hybrid", limit=None):
        require_text(query, "query", MAX_QUERY_CHARS)
        clamp_limit(limit)
        V8StdIndex._validate_types(types)
        V8StdIndex._validate_collections(collections)
        V8StdIndex._validate_mode(mode)
        generation = self._current()
        return self._present(generation, generation.index.search(
            query, types=types, collections=collections, mode=mode, limit=limit))

    def _compact(self, method, lookup, **kwargs):
        require_text(lookup, "id_or_alias_or_url", MAX_ID_OR_ALIAS_CHARS)
        generation = self._current()
        result = getattr(generation.index, method)(self._lookup(generation, lookup), **kwargs)
        return self._present(generation, result)

    def summary(self, id_or_alias_or_url, *, body_limit=2000):
        return self._compact("summary", id_or_alias_or_url, body_limit=body_limit)

    def section(self, id_or_alias_or_url, heading, *, body_limit=6000):
        return self._compact("section", id_or_alias_or_url, heading=heading, body_limit=body_limit)

    def pattern(self, id_or_alias_or_url, *, body_limit=6000):
        return self._compact("pattern", id_or_alias_or_url, body_limit=body_limit)

    def api_card(self, module_or_alias, member=None, *, body_limit=6000):
        return self._compact("api_card", module_or_alias, member=member, body_limit=body_limit)

    def related_ids(self, id_or_alias_or_url, *, relations=None, limit=None):
        return self._compact("related_ids", id_or_alias_or_url, relations=relations, limit=limit)

    def requirements_for_context(self, context, *, mechanisms=None, collections=None,
                                 limit=None, body_limit_per_item=1000):
        generation = self._current()
        return self._present(generation, generation.index.requirements_for_context(
            context, mechanisms=mechanisms, collections=collections,
            limit=limit, body_limit_per_item=body_limit_per_item))
